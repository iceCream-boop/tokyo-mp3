...
module.exports = {
  info: {
    name: "queue",
    description: "Mostra todas as músicas adicionadas a lista",
    usage: "",
    aliases: ["q", "lista"],
  },
  ...
  const permissions = message.channel.permissionsFor(message.client.user);
    ...
    if (!queue) return sendError("A lista está vazia ",message.channel)
...
function generateQueueEmbed(message, queue) {
  ...
  for (let i = 0; i < queue.length; i += 10) {
    ...
    .setFooter("Volume atual:" +serverQueue.volume)
     if(serverQueue.songs.length === 1)embed.setDescription("A lista está vazia")

...
module.exports = {
	info: {
		...
		usage: "<URL da playlist do YouTube> | <Nome da playlist>",
		aliases: ["pl"],
	},
		...
		if (url.match(/^https?:\/\/(www.youtube.com|youtube.com)\/playlist(.*)$/)) {
			try {
				...
				return message.channel.send({
					embed: {
						color: "PURPLE",
						description: `✅  **|** A playlist: **\`${videos[0].title}\`** foi adicionada lista`
					}
				})
			} catch (error) {
				console.error(error);
				return sendError("Playlists não encontrada",message.channel).catch(console.error);
			}
		} else {
			try {
				...
				let thing = new MessageEmbed()
					.setAuthor("Playlists adicionada")
					.setThumbnail(songInfo.thumbnail)
					.setColor("PURPLE")
					.setDescription(`✅  **|** A Playlist: **\`${songInfo.title}\`** adicionou \`${songInfo.videoCount}\` músicas a lista`)
				return message.channel.send(thing)
			} catch (error) {
				return sendError("*&$An u$nexpected error has o$_ccurr&ed",message.channel).catch(console.error);
			}
		}
...
async	function play(guild, song) {
  ...
  let afk = JSON.parse(fs.readFileSync("./afk.json", "utf8"));
    ...
    if (!song){
      if (!online.afk) {
        sendError("Como a lista de músicas está vazia e não há uma música tocando, eu ai do canal de voz", message.channel)
...
let thing = new MessageEmbed()
				.setAuthor("Tocando agora")//raubuserf")
				.setThumbnail(song.img)
				.setColor("PURPLE")
				.addField("Nome", song.title, true)
				.addField("Duração", song.duration, true)

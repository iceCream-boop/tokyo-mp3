...
module.exports = {
  info: {
    name: "play",
    description: "Para tocar músicas ou adicionar a playlist",
   ...
   var serverQueue = message.client.queue.get(message.guild.id);
    ...
    if (url.match(/^(https?:\/\/)?(www\.)?(m\.)?(youtube\.com|youtu\.?be)\/.+$/gi)) {
       try {
         ...
         if(!songInfo)return sendError("Eu não consegui encontrar a música no YouTube", message.channel);
        song = {
    ...
    if(searched.videos.length === 0)return sendError("Eu não consegui encontrar a música no YouTube", message.channel)
...
stream.on('error', function(er)  {
    ...
    try {
      ...
      message.client.queue.delete(message.guild.id);
      await channel.leave(); 
      return sendError(`Eu não posso entrar no canal de voz: ${error}`, message.channel);
    }
